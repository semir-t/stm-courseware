...
void initUSART2(uint32_t baudrate)
{
	...
	if(txMutex == NULL)
	{
		printUSART2("-> USART2: Error creating Mutex\n");
		while(1);
	}
}
...
uint8_t lockUSART2(void)
{
	if(xSemaphoreTake(txMutex,100) == pdTRUE)
	{
		return 1;
	}
	else
...
// {/// print one character to USART2
// 	uint8_t data;
// 	while(HAL_UART_Receive(&UART_HandleStruct, & data,1,10000) != HAL_OK);
// 	return data;
// }

uint8_t getcharUSART2(uint8_t * data)
{/// print one character to USART2
	uint8_t tmp;
	if(HAL_UART_Receive(&UART_HandleStruct, data,1,100) == HAL_OK)
	{
		return 1;
	}
	else
